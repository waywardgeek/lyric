...
### 1.1 `lyric fmt` lexer bug — keywords in strings
**Reference says**: "keywords inside string literals are tokenized as keywords, causing parse failures."
**Impact**: Can't format any .ly file containing `doc`, `source`, `match`, etc. in string literals. This is a showstopper for dogfooding.
...
**Bootstrap impact**: The lowerer.ly has many cases where you just want to check a variant kind without extracting data. Currently requires a full `match` with a wildcard arm, or a helper function.
...
**Bootstrap impact**: Verbose. The bootstrap .ly files are littered with this pattern. Go has no ternary either, but Rust does (`if cond { a } else { b }` is an expression). Since Lyric already has `match` expressions, making `if/else` an expression is consistent.
...
**This is wrong behavior.** In every language with `?` (Rust, Swift), the whole point is that `?` propagates the error and gives you the unwrapped value. If Lyric's `?` still leaves you with an optional, it's not useful — you've just moved the error check without gaining anything. The bootstrap lowerer.ly has `x!` after `?` everywhere, confirming this is painful.
...
`ExprSlice` exists in the AST but I don't see it in any testdata .ly file compiled through C. May be lowered/monomorphized correctly but unverified.

### 1.7 Import system is skeletal
`TImport` token exists, parser test exists, but no real module resolution. The bootstrap uses `lyric BlockName {}` wrappers + `MergeStdlib` + multi-file compilation on the command line. This works for bootstrap but is a gap.
...
**Observation**: The bootstrap .ly files rarely use `pub` because everything is in the same compilation unit via `MergeStdlib`. Once modules exist, visibility will matter a lot. Is the current default correct?
**Note**: Go's uppercase=public is unusual; Rust/Swift/Kotlin all use explicit `pub`/`public`. Lyric's choice aligns with modern languages.
...
**Bootstrap observation**: The lowerer.ly rarely needs the index. The Go-style forced two-variable form adds friction when you only want values.
...
**Observation**: This is clever but fragile. The testdata/generators.ly exists but the C backend implementation is limited. Is this a priority for bootstrap, or can it be deferred?
...
- Reference says "Next to port: optimizer.ly, monomorphizer.ly, c_backend.ly" — still accurate but should note the milestone we've reached

### 3.4 `.lyric` files are stale
The `pkg/ast/ast.lyric`, `pkg/checker/checker.lyric`, etc. are 27KB+ and likely haven't been updated since the massive changes (if-let, let-else, deepCopy, validator, etc.). These need a refresh in Phase 2.
...
The lowerer.ly is 1411 lines. A huge portion is `match expr.kind { ... }` with full destructuring. `if let` helps but isn't used throughout yet. The rewrite in Phase 2 should aggressively use `if let`/`let..else`.
...
The spec describes `File`, `Reader`, `Writer`, `BufferedReader` etc. but `stdlib/` only has `std.ly` and `string.ly`. No I/O stdlib exists yet. Bootstrap will need file I/O for the `main.ly` entry point.

### 5.3 Missing: string→number parsing
`atoi`/`parse_float` exist as builtins but aren't in the stdlib .ly files. They're hardcoded in the C backend.
...
- **Bootstrap .ly files are readable.** Despite being 8,049 lines total, the code is clear and well-structured.
...
2. **Fix `lyric fmt` string literal bug** — needed for dogfooding
...
6. **Refresh `.lyric` files** for all packages
7. **Add file I/O stdlib** — needed before `main.ly`
