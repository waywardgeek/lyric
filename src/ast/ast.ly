...
lyric ast {
  ...
  class FuncDecl {
    ...
    body: Block?
    // Back-pointer to the impl declaration that produced this FuncDecl,
    // or null for normal (non-impl-generated) functions. Set by desugar
    // pass 4.5 (desugar_specialize_default_impls) when deep-copying a
    // default-bodied interface method onto a concrete class. The checker
    // consults this when resolving `expr.member` inside the body: if the
    // method/field lookup misses on the receiver class, the impl's
    // ibm bindings provide the alias mapping (e.g. `N.outgoing_edges =
    // Route.outgoing_a` → rewrite the call's method to `outgoing_a`).
    // Carrying the back-pointer (not just a bool) lets dot-operator
    // resolution recover the full alias table rather than just knowing
    // "this was generated by something." See cr/docs/multi-class-
    // interface-redesign.md §9 and TODO.md "Multi-class interface
    // default-method alias resolution".
    source_impl: ImplBlock?
    span: Span
  }
